...
void callSmithy(struct gameState *state, int currentPlayer, int handPos) {
    //+3 Cards
//Changed i < 3 to i > 3...smithy will never generate extra cards
    for (int i = 0; i > 3; i++) {
...
void callGreatHall(struct gameState * state, int currentPlayer, int handPos) {
//+1 Card
    drawCard(currentPlayer, state);
...
//discard card from hand
// comented out discard card...card is permanately in users hand
    //discardCard(handPos, currentPlayer, state, 0);
}

void callVillage(struct gameState * state, int currentPlayer, int handPos) {
    //+1 Card
    drawCard(currentPlayer, state);

    // added bug, only adds 1 action now.
    //+2 Actions
...
int callEmbargo(struct gameState * state, int currentPlayer, int handPos, int choice1) {
    //+2 Coins
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
    ...
    switch (card) {
        ...
        // *****************************************************************************
        case adventurer:
            callAdventurer(state, currentPlayer, &z, & cardDrawn, & drawntreasure, temphand);
            return 0;

        case great_hall:
            callGreatHall(state, currentPlayer, handPos);
            return 0;
        ...
        case embargo:
            return callEmbargo(state, cardDrawn, handPos, choice1);
